...
if (isset($_POST["svc_no"])) {
    ...
    for ($count = 0; $count < count($svc_no); $count++) {
        ...
        if ($svc_no_clean != '') {
            $query .= 'INSERT INTO new_users(bd_no) VALUES("' . $svc_no_clean . '") ON DUPLICATE KEY UPDATE type_of_visit="de_brief_training",visit_country="' . $visiting_country . '",user_active="active",group_id="'.$group_id.'";';
        }
    }
