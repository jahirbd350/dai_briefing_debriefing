...
$personelInfo = mysqli_query($link, "SELECT * FROM new_users WHERE bd_no='$bd_no'");
$personelInfo = mysqli_fetch_assoc($personelInfo);

$visitInfo = mysqli_query($link, "SELECT * FROM briefing_20387_visit_info WHERE bd_no='$bd_no'&&visit_info_id='$visit_id'");
...
$mpdf->MultiCell(0, 10, 'বি বা ফরম-২০৩৮৫', 0, 'R');


$mpdf->SetFont('', 'B'.'U', 18);
$mpdf->MultiCell(0, 0, 'বিদেশ গমন প্রাক্কালে গোয়েন্দা ব্রীফিং', 0, 'C');
$mpdf->MultiCell(0, 0, '_____________________________', 0, 'C');

$mpdf->SetFont('', '', 13);
$mpdf->MultiCell(0, 12, '(ক্রমিক নং ১ হতে ৯ পর্যন্ত গমনকারী কর্তৃক পূরণীয়)', 0, 'C');
// Line break
$mpdf->Ln(5);
$mpdf->SetFont('', '', 14);
...
$mpdf->Ln(10);
$mpdf->SetFont('', '', 14);
$mpdf->MultiCell(100, 0, '৩ ।   নাম       ');
$mpdf->SetFont('', 'B', 12);
$mpdf->MultiCell(100, 0, '                           ' . $personelInfo['name']);
...
$mpdf->MultiCell(0, 0, '                                                                                                                        ' . $visitInfo['unit']);
$mpdf->MultiCell(0, 0, '                        _____________________________________________                      ___________________________________');
...
$mpdf->SetFont('', 'B', 12);
$mpdf->MultiCell(0, 0, '                                       ' . $visitInfo['purpose_of_visit']);
...
$fromDate = date('d M y',strtotime($visitInfo['date_of_reaching_destination']));
$toDate = date('d M y',strtotime($visitInfo['date_of_leaving_destination']));
$mpdf->Ln(10);
$mpdf->SetFont('', '', 14);
...
$departureDate = date('d M y',strtotime($visitInfo['departure_date']));
$mpdf->Ln(10);
$mpdf->SetFont('', '', 14);
$mpdf->MultiCell(0, 0, '৯ ।    বিদেশে গমনের তারিখ     ');
$mpdf->SetFont('', 'B', 12);
$mpdf->MultiCell(0, 0, '                                                               ' . $departureDate);
$mpdf->MultiCell(0, 0, '                                             ________________________________________________________________________________');
...
$mpdf->SetFont('', 'B', 14);
$mpdf->MultiCell(0, 0, '                                                                 প্রয়োজনীয় ব্রীফিং দেয়া হয়েছে ।');
