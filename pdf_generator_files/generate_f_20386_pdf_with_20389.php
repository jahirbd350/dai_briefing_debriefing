...
$personelData = mysqli_query($link, "SELECT * FROM new_users WHERE bd_no='$bd_no'");
$personelInfo = mysqli_fetch_assoc($personelData);
$visitData = mysqli_query($link, "SELECT * FROM debrief_20389_training_info WHERE bd_no='$bd_no'&&visit_info_id='$visit_id'");
$visitInfo = mysqli_fetch_assoc($visitData);
...
$mpdf->SetFont('', 'B', 12);
$mpdf->MultiCell(0, 0, '                           ' . $personelInfo['bd_no'] . '                                                                                     ' . $personelInfo['rank']);
...
$mpdf->SetFont('', 'B', 12);
$mpdf->MultiCell(0, 0, '                           ' . $personelInfo['br_trade']. '                                                                                   ' . $personelInfo['unit'] );
$mpdf->MultiCell(0, 0, '                        _____________________________________________                      ___________________________________');
...
$mpdf->SetFont('', 'B', 12);
$mpdf->MultiCell(0, 0, '                                       ' . $visitInfo['name_of_course']);
...
$mpdf->SetFont('', 'B', 12);
$mpdf->MultiCell(0, 0, '                                                               ' . $visitInfo['course_which_country']);
